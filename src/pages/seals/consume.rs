            ...
            div { class: "{card_class()} p-6 space-y-5",
                ...
                button {
                    onclick: move |_| {
                        ...
                        result.set(None);
                        error.set(Some(
                            "Seal consumption requires a configured CSV runtime host. The wallet will not mark a seal consumed locally.".to_string(),
                        ));
                    },
                    ...
                    if available_seals.is_empty() { "No Available Seals" } else { "Consume Seal via Runtime" }
                }
            }
        }
    }
