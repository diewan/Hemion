    ...
    rsx! {
        div { class: "max-w-2xl space-y-6",
            ...
            div { class: "{card_class()} p-6 space-y-5",
                ...
                button {
                    onclick: move |_| {
                        if wallet_ctx.get_sanad(&sanad_id.read()).is_none() {
                            result.set(Some("Sanad not found.".to_string()));
                        } else {
                            result.set(Some("Sanad consumption requires a configured CSV runtime host. Local projections are not changed.".to_string()));
                        }
                    },
