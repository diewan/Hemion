...
mod tests {
    ...
    fn test_backup_creation() {
        let test_dir =
            std::env::temp_dir().join(format!("csv-test-keystore-backup-{}", uuid::Uuid::new_v4()));
        let test_dir = test_dir.to_string_lossy().into_owned();
        let policy = SecurityPolicy {
            enable_auto_backup: false,
            ..SecurityPolicy::default()
        };
        let mut keystore = NativeKeystore::with_dir_and_policy(&test_dir, policy).unwrap();
        let secret_key = test_secret_key();
        let passphrase = test_passphrase();
